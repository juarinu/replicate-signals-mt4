...
//+------------------------------------------------------------------+
#property copyright "Replicar Sinais - Created by Jam S�vio - Client"
...
void Validacao(){
  ...
  if (Cursorrr >= 0) {
      ...
      for (iii=0; iii<Rowsss; iii++) 
        if (MySqlCursorFetchRow(Cursorrr)) {
          ...
          if ((vDia-dia)<=5 && (vDia-dia)>0 && vMes==mes && vAno==ano) {
            txtValid = "Favor entrar em contato para renov�-la.";
            txtValid = vNome+", faltam "+((vDia)-dia)+" dia(s) para expirar sua licen�a."; 
          }
          
          if((vDia-dia)<=0 && (vMes-mes)<=0 && (vAno-ano)<=0){
            txtValid = "Desculpe "+vNome+", sua licen�a expirou.";
            status=false;
          }
          ...
          if ((vDia-dia)>=6 || vMes>=mes || vAno>=ano) {
            if (vMes>mes || vAno>ano) {
              ...
              txtValid=vNome+", faltam "+cal+" dia(s) para expirar sua licen�a."; 
            }else{
              txtValid=vNome+", faltam "+((vDia)-dia)+" dia(s) para expirar sua licen�a."; 
            }
          }
    ...
    status=false;
    txtValid="Voc� n�o possui licen�a para receber estes sinais."; 
  }
} 
  ...
  if (Cursor >= 0) {
    ...
    for (i=0; i<Rows; i++)
      if (MySqlCursorFetchRow(Cursor)) {
        ...
        if (contador==0) {
          // Calcula o lote   
          if (vLote==0.01 && AccountBalance()<vSaldo) {
            Lote = vLote;
          } else {
    ...
    Print ("#Sinais - Cursor opening failed. Error: ", MySqlErrorDescription); 
  }
  ...
  for (int i=0; i<=OrdersTotal(); i++) {
    if(OrderSelect(i,SELECT_BY_POS,MODE_TRADES)){
      ...
      if (Cursorr >= 0) {
        ...
        Print ("#VerificaStop - Cursor opening failed. Error: ", MySqlErrorDescription);
      }
    }
  }
}
...
//+------------------------------------------------------------------+
//| Verifica se a ordem est� aberta                                   |
//+------------------------------------------------------------------+
int VerificaOrdens() { 
  if (OrdersTotal()>0) {
    string Query;
    ...
    for (int i=0; i<=OrdersTotal(); i++) {
      if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES)) {
        ...
        if (Cursor >= 0) {
          ...
          Print ("#VerificaOrdens - Cursor opening failed. Error: ", MySqlErrorDescription);
        }
      }
    }
  }
